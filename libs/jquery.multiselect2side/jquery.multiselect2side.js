	...
	var methods = {
		init: function(options) {
			...
			return this.each(function() {
				...
				if (!data) el.data('multiselect2side', o);

				var originalName = $(this).attr("name");
				if (originalName.indexOf('[') != -1) originalName = originalName.substring(0, originalName.indexOf('['));
				...
				if (o.search != false && o.optGroupSearch != false) {
					...
					if (o.selectedPosition == 'right') leftSearch = ss;
					else rightSearch = ss;
				...
				if (o.optGroupSearch != false) {
					...
					el.children("optgroup").each(function() {
						if (searchSelect.find("[value='" + $(this).attr("label") + "']").size() == 0) searchSelect.append("<option value='" + $(this).attr("label") + "'>" + $(this).attr("label") + "</option>");
					});
					searchSelect.change(function() {
						...
						if (sEl.val() != lastOptGroupSearch) {
							...
							setTimeout(function() {
								...
								} else els = el.find("optgroup[label='" + sEl.val() + "']").children("option:not(:selected)");
				...
				var fto = function() {
					...
					if (searchV == searchInput.val()) return;
					...
					setTimeout(function() {
						...
						if (searchV == "") {
							...
							toSearch.each(function() {
								...
								if (o.caseSensitive) find = myText.indexOf(searchV);
								else find = myText.toUpperCase().indexOf(searchV.toUpperCase());

								if (find != -1) $(this).clone().appendTo(leftSel).removeAttr("selected");
							});

							if (leftSel.children().length == 0) searchInput.css({
				...
				$(this).next().find('.ms2side__options, .ms2side__updown').each(function() {
					...
					if (top > 0) $(this).css('padding-top', top + 'px');
				})
				...
				var nLastAutosort = 0;
				if (o.autoSort) allSel.change(function() {
				...
				if (o.autoSortAvailable) allSel.change(function() {
				...
				allSel.change(function() {
					...
					if ($.browser.msie && $.browser.version == '6.0') el.show().hide();
					...
					if (selectedSx.size() == 0 || (o.maxSelected >= 0 && (selectedSx.size() + selectDx.size()) > o.maxSelected)) div.find(".AddOne").addClass('ms2side__hide');
					else div.find(".AddOne").removeClass('ms2side__hide');

					// FIRST HIDE ALL
					div.find(".RemoveOne, .MoveUp, .MoveDown, .MoveTop, .MoveBottom, .SelSort").addClass('ms2side__hide');
					if (selectDx.size() > 1) div.find(".SelSort").removeClass('ms2side__hide');
					...
					if (selectSx.size() == 0 || (o.maxSelected >= 0 && selectSx.size() >= o.maxSelected)) div.find(".AddAll").addClass('ms2side__hide');
					else div.find(".AddAll").removeClass('ms2side__hide');

					if (selectDx.size() == 0) div.find(".RemoveAll").addClass('ms2side__hide');
					else div.find(".RemoveAll").removeClass('ms2side__hide');
				});
				...
				$(this).next().find('.ms2side__options').children().click(function() {
					if (!$(this).hasClass("ms2side__hide")) {
						if ($(this).hasClass("AddOne")) {
							...
							if (removeFilter.is(":visible") || (searchSelect.length > 0 && searchSelect.val() != "__null__")) leftSel.children().each(function(i, selected) {
								$(this).remove().appendTo(rightSel);
								el.find("[value='" + $(selected).val() + "']").remove().appendTo(el).attr("selected", true);
							});
							else {
